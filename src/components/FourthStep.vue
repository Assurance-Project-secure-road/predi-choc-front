        ...
        <div class="card-body">
            ...
            <div class="mb-3">
                <label
                    for="TypeMeteo"
                    class="form-label"
                >Sous quelle condition météo avez-vous l’habitude de conduire ?</label>
                <div class="form-check" v-for="(typeMeteo, i) in typesMeteo" :key="'meteo_' + i">
                    <input
                        ...
                        :value="i + 1"
                        :id="'meteo_' + i"
                        v-model="meteoSelected"
                    />
                    <label class="form-check-label" :for="'meteo_' + i">{{ typeMeteo }}</label>
                </div>
            </div>
            <div class="mb-3">
                <label
                    for="motifDeplace"
                    class="form-label"
                >Quel type de trajet faites-vous le plus avec votre véhicule?</label>
                <select
                    class="form-select"
                    aria-label="Default"
                    id="motifDeplace"
                    v-model="motifDeplace"
                >
                    <option disabled value>Choisissez un type ...</option>
                    <option
                        v-for="(motif, k) in lsMotifDeplacement"
                        :value="k + 1"
                        :key="'deplacement_' + k"
                    >{{ motif }}</option>
                </select>
            ...
            <div class="mb-3">
                <label
                    for="departementsVoyage"
                    class="form-label"
                >Avez vous l'habitude de voyager dans d'autres departement ?</label>
                <div class="row">
                    <div
                        class="form-check col-12 col-md-3"
                        v-for="(dpt, i) in departement"
                        :key="'departement_' + i"
                    >
                        <input
                            class="form-check-input"
